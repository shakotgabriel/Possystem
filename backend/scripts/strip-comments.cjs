...
function stripJsxBraceComments(text) {
  // Removes occurrences like: { /* ... */ } including the braces.
  // Keeps only line breaks so line numbers roughly match.
  let out = ''
  let i = 0
...
function isPreservedDirectiveComment(rawCommentText) {
  // Keep TypeScript directives to avoid breaking builds.
  // - Triple-slash directives: /// <reference ... />, /// <amd-module ...>, etc.
...
async function main() {
  ...
  for (const file of files) {
    // Skip this tool script itself
    if (path.basename(file) === 'strip-comments.cjs') continue
